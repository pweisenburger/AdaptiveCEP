...
import adaptivecep.data.Queries.SelfJoinQuery
import adaptivecep.graph.nodes.implementation.SelfJoinNodeImpl
...
case class SelfJoinNodeProxy(
  ...
  override def receive: Receive = {
    ...
    case event: Event =>
      if(sender() == worker) context.parent ! event else worker.forward(event)
    case Update(clazz) =>
  ...
  override def preStart(): Unit = {
    ...
    worker = context.actorOf(Props(SelfJoinNodeImpl(this.query, publishers, frequencyMonitorFactory,
      latencyMonitorFactory, createdCallback, eventCallback, childNode)))
    super.preStart()
  }
