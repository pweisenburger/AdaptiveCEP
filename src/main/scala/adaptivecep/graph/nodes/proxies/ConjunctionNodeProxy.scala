...
import adaptivecep.graph.nodes.implementation.ConjunctionNodeImpl
...
case class ConjunctionNodeProxy(
  ...
  override def preStart(): Unit = {
    ...
    worker = context.actorOf(Props(ConjunctionNodeImpl(query, publishers, frequencyMonitorFactory,
      latencyMonitorFactory, createdCallback, eventCallback, childNode1, childNode2)))
    super.preStart()
  }
