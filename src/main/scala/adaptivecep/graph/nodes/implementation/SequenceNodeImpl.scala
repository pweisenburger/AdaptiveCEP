...
case class SequenceNodeImpl(
  ...
  def this(args: Tuple6[SequenceQuery, Map[String, ActorRef],
  ...
  nodeData = LeafNodeData(name, query, context)

  queryPublishers.foreach(_ ! Subscribe)

  addEventType("sq1", SequenceNodeImpl.createArrayOfNames(query.s1), SequenceNodeImpl.createArrayOfClasses(query.s1))
  addEventType("sq2", SequenceNodeImpl.createArrayOfNames(query.s2), SequenceNodeImpl.createArrayOfClasses(query.s2))
