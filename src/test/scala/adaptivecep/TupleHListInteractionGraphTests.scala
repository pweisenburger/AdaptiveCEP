...
class TupleHListInteractionGraphTests extends GraphTestSuite {
  ...
  test("Tuple/HList - SequenceNode - 3") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val query: Query[(Int, String, String)] =
      sequence(nStream[Int]("A") -> nStream[(String, String)]("B"))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    a ! Event(21)
    Thread.sleep(2000)
    b ! Event("21", "42")
    expectMsg(Event(21, "21", "42"))
    stopActors(a, b, graph)
  }

  test("Tuple/HList - SequenceNode - 4") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val query: Query[(Int, Int, String)] =
      sequence(nStream[(Int, Int)]("A") -> nStream[String]("B"))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    a ! Event(21, 42)
    Thread.sleep(2000)
    b ! Event("42")
    expectMsg(Event(21, 42, "42"))
    stopActors(a, b, graph)
  }

  test("Tuple/HList - SequenceNode - 5") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val query: Query[Int::String::HNil] =
      sequence(nStream[Int]("A") -> nStream[String::HNil]("B"))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    a ! Event(42)
    Thread.sleep(2000)
    b ! Event("21")
    expectMsg(Event(42, "21"))
    stopActors(a, b, graph)
  }

  test("Tuple/HList - SequenceNode - 6") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val query: Query[Int::Int::String::HNil] =
      sequence(nStream[Int::Int::HNil]("A") -> nStream[String]("B"))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    a ! Event(21, 42)
    Thread.sleep(2000)
    b ! Event("21")
    expectMsg(Event(21, 42, "21"))
    stopActors(a, b, graph)
  }

  test("Tuple/HList - SequenceNode - 7") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val query: Query[(Int, String)] =
      sequence(nStream[Int]("A") -> nStream[String]("B"))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    a ! Event(42)
    Thread.sleep(2000)
  ...
  test("Tuple/HList - JoinOnNode - 3") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val sq: Query[Int::Int::HNil] = stream[Int::Int::HNil]("B")
    val query: Query[Int::Int::HNil] =
      stream[Int]("A")
        .joinOn(sq, Nat._1, Nat._1, tumblingWindow(3.instances), tumblingWindow(2.instances))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    a ! Event(1)
    a ! Event(2)
  ...
  test("Tuple/HList - JoinOnNode - 4") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val sq: Query[Int] = stream[Int]("B")
    val query: Query[(String, Boolean, Int)] =
      stream[(String, Boolean, Int)]("A")
        .joinOn(sq, Nat._3, Nat._1, tumblingWindow(3.instances), tumblingWindow(2.instances))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    ...
    b ! Event(5)
    b ! Event(7)
    expectMsg(Event("a", true, 1))
    expectMsg(Event("e", true, 3))
    stopActors(a, b, graph)
  }

  test("Tuple/HList - JoinOnNode - 5") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val sq: Query[(Int, Int)] = stream[(Int, Int)]("B")
  ...
  test("Tuple/HList - DisjunctionNode - 3") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val query: Query[(Either[Int, String], Either[Unit, String])] =
      stream[Int]("A")
        .or(stream[(String, String)]("B"))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    ...
    Thread.sleep(2000)
    b ! Event("21", "42")
    expectMsg(Event(Left(21), Left(())))
    expectMsg(Event(Right("21"), Right("42")))
    stopActors(a, b, graph)
  }

  test("Tuple/HList - DisjunctionNode - 4") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val query: Query[(Either[Int, String], Either[Int, Unit])] =
      stream[(Int, Int)]("A")
        .or(stream[String]("B"))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b), testActor)
    expectMsg(Created)
    ...
    b ! Event("42")
    expectMsg(Event(Left(21), Left(42)))
    expectMsg(Event(Right("42"), Right(())))
    stopActors(a, b, graph)
  }

  test("Tuple/HList - DisjunctionNode - 5") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val c: ActorRef = createTestPublisher("C")
    val query: Query[Either[Either[Int, String], Boolean]:: Either[Either[Unit, String], Boolean]:: Either[Unit,Boolean]::HNil] =
      stream[Int]("A")
        .or(stream[String::String::HNil]("B"))
    ...
    Thread.sleep(2000)
    c ! Event(true, false, true)
    expectMsg(Event(Left(Left(21)), Left(Left(())), Left(())))
    expectMsg(Event(Left(Right("21")), Left(Right("42")), Left(())))
    expectMsg(Event(Right(true), Right(false), Right(true)))
    stopActors(a, b, c, graph)
  }

  test("Tuple/HList - DisjunctionNode - 6") {
    val a: ActorRef = createTestPublisher("A")
    val b: ActorRef = createTestPublisher("B")
    val c: ActorRef = createTestPublisher("C")
    val query: Query[Either[Either[Int, String], Boolean]:: Either[Either[Int, String], Unit]::HNil] =
      stream[Int::Int::HNil]("A")
        .or(stream[(String, String)]("B"))
        .or(stream[Boolean]("C"))
    val graph: ActorRef = createTestGraph(query, Map("A" -> a, "B" -> b, "C" -> c), testActor)
    ...
    Thread.sleep(2000)
    c ! Event(true)
    expectMsg(Event(Left(Left(21)), Left(Left(42))))
    expectMsg(Event(Left(Right("21")), Left(Right("42"))))
    expectMsg(Event(Right(true), Right(())) )
    stopActors(a, b, c, graph)
  }
